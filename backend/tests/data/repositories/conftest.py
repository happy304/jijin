"""Shared fixtures for repository layer tests.

Uses SQLite in-memory mode (via aiosqlite) so no live PostgreSQL is
required. The Alembic migration is applied once per test session to
create all tables, then each test gets a fresh transaction that is
rolled back on teardown — keeping tests isolated and fast.

Requirements: 2.6, 1.11
"""

from __future__ import annotations

from collections.abc import AsyncIterator
from pathlib import Path

import pytest
from sqlalchemy.ext.asyncio import AsyncSession, async_sessionmaker, create_async_engine

from app.core.config import Settings
from app.data.migrations import run_upgrade
from app.data.models import Base


# ---------------------------------------------------------------------------
# SQLite in-memory engine (shared across the test session)
# ---------------------------------------------------------------------------

_SQLITE_URL = "sqlite+aiosqlite:///:memory:"
_SQLITE_SYNC_URL = "sqlite:///:memory:"


def _test_settings(db_path: str) -> Settings:
    """Build Settings pointing at a temporary SQLite file."""
    return Settings(
        _env_file=None,  # type: ignore[call-arg]
        DATABASE_SYNC_URL=f"sqlite:///{db_path}",
        DB_AUTO_MIGRATE="true",
    )


@pytest.fixture(scope="session")
def sqlite_db_path(tmp_path_factory: pytest.TempPathFactory) -> str:
    """Create a single SQLite file for the whole test session."""
    db_file = tmp_path_factory.mktemp("db") / "test_repos.db"
    settings = _test_settings(str(db_file))
    run_upgrade(settings, "head")
    return str(db_file)


@pytest.fixture
async def session(sqlite_db_path: str) -> AsyncIterator[AsyncSession]:
    """Yield an AsyncSession backed by the session-scoped SQLite DB.

    Each test runs inside a SAVEPOINT so changes are rolled back
    automatically, keeping tests isolated without recreating the schema.
    """
    engine = create_async_engine(
        f"sqlite+aiosqlite:///{sqlite_db_path}",
        echo=False,
        future=True,
    )
    factory = async_sessionmaker(
        bind=engine,
        class_=AsyncSession,
        expire_on_commit=False,
        autoflush=False,
    )
    async with factory() as sess:
        async with sess.begin():
            yield sess
            await sess.rollback()

    await engine.dispose()
